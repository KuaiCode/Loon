...
// === Loon / Quantumult X 兼容层 ===
// 目的：让原 Quantumult X 脚本在 Loon 中也能使用同一套业务代码。
// QX 原生：$prefs / $task.fetch / $notify
// Loon 原生：$persistentStore / $httpClient / $notification.post
(function () {
  const g = typeof globalThis !== 'undefined' ? globalThis : this;

  if (typeof g.$prefs === 'undefined' && typeof g.$persistentStore !== 'undefined') {
    g.$prefs = {
      valueForKey: function (key) {
        return g.$persistentStore.read(key);
      },
      setValueForKey: function (value, key) {
        return g.$persistentStore.write(String(value), key);
      }
    };
  }
  ...
  if (typeof g.$notify === 'undefined' && typeof g.$notification !== 'undefined') {
    g.$notify = function (title, subtitle, body) {
      g.$notification.post(String(title || ''), String(subtitle || ''), String(body || ''));
    };
  }
    ...
    g.$task = {
      fetch: function (request) {
        ...
        return new Promise(function (resolve, reject) {
          ...
          if (request.timeout) params.timeout = request.timeout;

          const done = function (error, response, data) {
            if (error) {
          ...
          if (typeof g.$httpClient[method] === 'function') {
            g.$httpClient[method](params, done);
          } else {
            g.$httpClient.get(params, done);
          }
        });
      }
    };
